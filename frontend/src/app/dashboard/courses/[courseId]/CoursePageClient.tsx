...
export default function CoursePageClient() {
  ...
  const load = useCallback(async () => {
    ...
    if (!courseRes.ok) {
      return { ok: false as const, message: courseRes.message, notFound: courseRes.notFound };
    }
    if (!quizRes.ok) {
      return { ok: false as const, message: quizRes.message, notFound: quizRes.notFound };
    }
    return {
      ok: true as const,
      data: { course: courseRes.course, quizzes: quizRes.quizzes },
    };
  }, [courseId]);
  ...
  return (
    <MarketingAuthShell mainVariant="wide">
      <div className="mx-auto w-full max-w-4xl px-2 py-4 sm:px-4">
        <DashboardBackLink href="/dashboard" label="← Zurück zum Dashboard" />

        <DashboardAsyncPage
          loading={loading}
          loadingLabel="Kurs wird geladen…"
          notFound={notFound}
          error={error}
